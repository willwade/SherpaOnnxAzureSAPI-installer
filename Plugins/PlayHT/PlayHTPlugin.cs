...
namespace Plugins.PlayHT
{
    /// <summary>
    /// Plugin adapter for PlayHT TTS engine
    /// </summary>
    public class PlayHTPlugin : TtsEngineBase
    {
        private readonly PlayHTEngine _engine;
        
        /// <summary>
        /// Initializes a new instance of the PlayHTPlugin class
        /// </summary>
        public PlayHTPlugin()
        {
            _engine = new PlayHTEngine();
        }
        ...
        {
            // Check if API key and user ID are provided
            if (!configuration.TryGetValue("apiKey", out string apiKey) || string.IsNullOrEmpty(apiKey))
            {
                return false;
            }
            
            if (!configuration.TryGetValue("userId", out string userId) || string.IsNullOrEmpty(userId))
            {
                return false;
            }
        ...
        public override async Task<IEnumerable<TtsVoiceInfo>> GetAvailableVoicesAsync(Dictionary<string, string> configuration)
        {
            ...
            {
                Console.WriteLine($"Error getting PlayHT voices: {ex.Message}");
